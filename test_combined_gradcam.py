#!/usr/bin/env python3
"""
Test script for combined Grad-CAM visualization
"""
...
def test_combined_gradcam():
    """Test the combined Grad-CAM functionality"""
    print("\n=== Testing Combined Grad-CAM ===")
    ...
    try:
        ...
        test_image_path = "test_combined_gradcam_image.png"
        ...
        try:
            densenet_results = apply_combined_gradcam(test_image_path, model_type='densenet', probability_threshold=0.5)
            print("✓ DenseNet combined Grad-CAM successful")
            ...
            print(f"  Heatmap shape: {densenet_results['heatmap'].shape}")
            ...
            plt.subplot(1, 3, 2)
            plt.imshow(densenet_results['heatmap'], cmap='jet')
            ...
            plt.tight_layout()
            plt.savefig('test_combined_gradcam_densenet.png', dpi=150, bbox_inches='tight')
            plt.close()
            print("✓ Saved DenseNet visualization to: test_combined_gradcam_densenet.png")
        ...
        try:
            resnet_results = apply_combined_gradcam(test_image_path, model_type='resnet', probability_threshold=0.5)
            print("✓ ResNet combined Grad-CAM successful")
            ...
            print(f"  Heatmap shape: {resnet_results['heatmap'].shape}")
            ...
            plt.subplot(1, 3, 2)
            plt.imshow(resnet_results['heatmap'], cmap='jet')
            ...
            plt.tight_layout()
            plt.savefig('test_combined_gradcam_resnet.png', dpi=150, bbox_inches='tight')
            plt.close()
            print("✓ Saved ResNet visualization to: test_combined_gradcam_resnet.png")
        ...
        try:
            low_threshold_results = apply_combined_gradcam(test_image_path, model_type='densenet', probability_threshold=0.3)
        ...
        print(f"✗ Error during combined Grad-CAM testing: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    """Main test function"""
    print("Combined Grad-CAM Test Suite")
    print("=" * 50)
    
    success = test_combined_gradcam()
    
    if success:
        print("\n✓ All tests completed successfully!")
        print("Combined Grad-CAM visualization is working correctly.")
    else:
        print("\n✗ Some tests failed!")
        sys.exit(1)
